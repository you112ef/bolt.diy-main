...
const buttonVariants = cva(
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-bolt-elements-borderColor disabled:pointer-events-none disabled:opacity-50 active:opacity-80',
  {
    variants: {
      variant: {
      ...
      size: {
        default: 'h-8 sm:h-9 px-3 sm:px-3 py-1.5 sm:py-1.5 text-xs sm:text-sm', // Adjusted default size
        sm: 'h-8 rounded-md px-3 text-xs', // Kept sm as is, it's already compact
        lg: 'h-9 rounded-md px-6', // Slightly reduced height and padding for lg
        icon: 'h-8 w-8', // Slightly reduced icon button size
      },
    },
    defaultVariants: {
      variant: 'default',
